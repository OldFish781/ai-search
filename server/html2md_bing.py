...
# Step 1: 在 Bing 上搜索关键词并获取链接
async def search_bing(keyword, max_results=5):
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)  # headless=False 可以观察浏览器操作
...
async def fetch_and_convert_to_md(url):
    async with async_playwright() as p:
        ...
        try:
            ...
            # 使用 Trafilatura 将 HTML 转换为 Markdown
...
async def main():
    ...
    # 获取 Bing 搜索结果
    search_results = await search_bing(keyword, max_results=max_results)
